...
The conditional edge reads `state["route"]`, which the router node sets. Phase 9
expands the supervisor branch into researcher / action-taker / reviewer; Phase 10
inserts the approval interrupt before any sensitive dispatch.
...
from app.graph.nodes import RouterNode, SimpleRagNode, SupervisorStubNode, select_branch
...
def build_graph_builder(
    ...
    *,
    ...
    builder.add_node("supervisor", SupervisorStubNode(chain))
